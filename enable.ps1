#####################################################
# HelloID-Conn-Prov-Target-Esis-Enable
#
# Version: 1.0.0
#####################################################
...
try {
    ...
    if ($dryRun -eq $true) {
        $auditLogs.Add([PSCustomObject]@{
                Message = "Enable Esis account for: [$($p.DisplayName)] will be executed during enforcement"
            })
    }

    if (-not($dryRun -eq $true)) {
        Write-Verbose "Enabling Esis account with accountReference: [$aRef]"
        ...
        $success = $true
        $auditLogs.Add([PSCustomObject]@{
                Message = "Enable account was successful, message $($enableDepartmentRequestResult.message)"
                IsError = $false
            })
    }
}
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $errorMessage = "Could not enable Esis account. Error: $($errorObj.ErrorMessage)"
    }
    else {
        $errorMessage = "Could not enable Esis account. Error: $($ex.Exception.Message)"
    }
