#################################################
# HelloID-Conn-Prov-Target-Esis_Employee-Permissions-Board-Role
# PowerShell V2
#################################################
...
try {
    Write-Information 'Starting board role entitlement import'
    ...
    $actionMessage = 'querying board roles'
    ...
    Write-Information "Queried board roles. Result count: $($esisRoles.Count)"

    $actionMessage = 'filtering for active board roles'
    $activeEsisRoles = $esisRoles | Where-Object { $_.isActief -eq $true } | Sort-Object -Property rolUniekId -Unique
    Write-Information "Filtered for active board roles. Result count: $($activeEsisRoles.Count)"

    $actionMessage = "importing board role permissions to HelloID"
    foreach ($activeEsisRole in $activeEsisRoles) {
        $actionMessage = "importing board role [$($activeEsisRole.rolNaam)] to HelloID"

        # Make sure displayname is not too long (max 100 characters)
        $displayName = "Board Role - $($activeEsisRole.rolNaam)"
        $displayName = $displayName.substring(0, [System.Math]::Min(100, $displayName.Length))
        $outputContext.Permissions.Add(
            @{
                displayName    = $displayName
                identification = @{
                    Id = $activeEsisRole.rolUniekId
                }
            }
        )
    }
