#################################################
# HelloID-Conn-Prov-Target-Esis_Employee-RevokePermission-Board-Role
# PowerShell V2
#################################################
...
try {
    ...
    $actionMessage = "revoking board role [$($actionContext.References.Permission.Id)] from account with AccountReference: [$($actionContext.References.Account)]"
    ...
    $revokePermissionSplatParams = @{
        uri         = "$($actionContext.Configuration.BaseUrl)/v1/api/gebruiker/$($actionContext.References.Account)/ontkoppelenvanrolopbestuur"
        Method      = 'DELETE'
        Headers     = $headers
    ...
    if (-Not($actionContext.DryRun -eq $true)) {
        ...
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                ...
                Message = "Revoked board role [$($actionContext.References.Permission.Id)] from account with AccountReference: [$($actionContext.References.Account)]. Message [$($revokePermissionRequestResult.message)]"
                IsError = $false
            })
    }
    else {
        Write-Information "[DryRun] Would revoke board role [$($actionContext.References.Permission.Id)] from account with AccountReference: [$($actionContext.References.Account)]"
    }
}
catch {
    $ex = $PSItem
    ...
    if ($auditMessage -like '*Gebruiker is niet gekoppeld aan de opgegeven rol*') {
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                Action  = "RevokePermission" # Optional
                Message = "Skipped $($actionMessage). Reason: User already does not have board role."
                IsError = $false
            })
    }
    else {
