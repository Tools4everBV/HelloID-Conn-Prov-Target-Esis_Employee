#####################################################
# HelloID-Conn-Prov-Target-Esis-Disable
#
# Version: 1.0.0
#####################################################
...
try {
    ...
    if ($dryRun -eq $true) {
        $auditLogs.Add([PSCustomObject]@{
                Message = "Disable Esis account for: [$($p.DisplayName)] will be executed during enforcement"
            })
    }

    if (-not($dryRun -eq $true)) {
        Write-Verbose "Disabling Esis account with accountReference: [$aRef]"
        ...
        $body = @{
            ...
            gebruikersNaam = "$($aRef)"
            brin6          = "$($config.Department)"
            functie        = "$($responseUser.medewerkers.aanstellingen.functieomschrijving)"
        } | ConvertTo-Json
        ...
        $success = $true
        $auditLogs.Add([PSCustomObject]@{
                Message = "Disable account was successful, message $($disableDepartmentRequestResult.message)"
                IsError = $false
            })
    }
}
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $errorMessage = "Could not disable Esis account. Error: $($errorObj.ErrorMessage)"
    }
    else {
        $errorMessage = "Could not disable Esis account. Error: $($ex.Exception.Message)"
    }
